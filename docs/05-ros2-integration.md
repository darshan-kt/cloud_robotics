...
**Post-Milestone-11 addition:** `docker-compose.yml`'s `robot` service passes the *host's* X11 display through unconditionally (`DISPLAY` env var + the `/tmp/.X11-unix` socket bind-mounted), and `simulation.launch.py` checks at launch-description-generation time: if `DISPLAY` is actually set, it launches `gzclient` (the GUI) alongside `gzserver` automatically - see root [`README.md`](../README.md)'s "Watching the simulation visually." It doesn't change any of the reasoning above: on a host with no `DISPLAY` (a real headless server, CI, a future AWS-hosted edge deployment) this is simply false and the simulation runs exactly as it always did, headless is still what actually ships, and there's still nothing to forward in AWS. It's a debugging window that opens itself when a display is available, not a dependency anything relies on.

Getting this working surfaced a real bug worth recording: `gazebo_ros`'s own `gzclient.launch.py` unconditionally injects `--gui-client-plugin=libgazebo_ros_eol_gui.so` (a Gazebo-Classic-deprecation-notice HUD overlay) into the `gzclient` command, with no argument to turn it off. Under this container's real resource constraints, that plugin caused `gzclient`'s CPU usage to climb continuously (observed: 139% → 166%+ over two minutes) without ever actually mapping a window - confirmed by direct comparison against a plain `gzclient` invocation with no extra plugin, which loads normally (a real "Preparing your world..." splash screen, then settles into steady-state CPU once the scene finishes loading). `simulation.launch.py` runs `gzclient` directly via `ExecuteProcess` instead of including `gazebo_ros`'s launch file, specifically to avoid that plugin - see that file's own comment for the full story.
...
- **`robot-container/ros_ws/src/robot_cloud_bridge/`** — new `ament_python` ROS2 package, the only place in the repo that imports `rclpy`:
  - `real_ros_adapter.py` — `RealROSAdapter(ROSAdapter)`, described above. `subscribe_lidar()` (post-Milestone-11) is real too, the same way `subscribe_odometry()` always was: `waffle_pi`'s built-in simulated LDS-01 publishes `/scan` natively, nothing extra to launch - see [`docs/api-reference.md`](api-reference.md)'s `lidar` MQTT topic entry and [`docs/09-frontend.md`](09-frontend.md) for where it ends up.
