...
**Post-Milestone-11 addition:** `docker-compose.gui.yml` is an opt-in override (see root [`README.md`](../README.md)'s "Watching the simulation visually") that passes the *host's* X11 display into the robot container so a `gzclient` can be attached from outside, on demand, purely for local development convenience while watching a robot get driven. It doesn't change any of the reasoning above — `simulation.launch.py` still never starts `gzclient` itself, headless is still what actually ships, and there's still nothing to forward in an AWS deployment. It's a debugging window someone can open, not a dependency anything relies on.
