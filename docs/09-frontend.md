...
> **Status: complete.** The React + TypeScript + Tailwind operator console is real, not a stub: login, a live fleet dashboard, a Robot page with actual decoding WebRTC video, a **LiDAR scan panel** (added post-Milestone-11 alongside the robot's `/scan` feed), arrow-button/keyboard teleop throttled to 20Hz, an emergency stop, Health and Settings pages. Verified end-to-end with a real, **unmodified** Chrome browser driven by Playwright - and getting there surfaced *three* genuine WebRTC integration bugs across two rounds of hardening (Milestone 9 itself, then a follow-up once heavier reconnect-cycle usage while building the LiDAR panel exposed a race the first fix had only narrowed). All three are fixed at the root, not worked around. See Verification below for the full story.
...
**Pages** (`src/pages/`) + `src/components/Layout.tsx`, `TeleopPad.tsx`, `StatusDot.tsx`, `LidarView.tsx`:
...
- `Robot` - live WebRTC video, the arrow-pad + keyboard teleop, take/release control, emergency stop (bypasses the session, same as the backend's own override), polled telemetry/health/**LiDAR** panels. `LidarView` (post-Milestone-11) is a plain `<canvas>` top-down scan plot - no charting library, the same "a native API already does the job" call as `<video>` for camera - drawn from `RobotDetail.lidar`, which rides the exact same 500ms poll (tightened from 2s when this panel was added - see `docs/api-reference.md`) as telemetry/health, not a new WebSocket.
...
### Loose end from the second pass: a leftover line that only a real webcam surfaced

Refactoring `_build_pipeline()`/`_prepare_fresh_webrtcbin()` (above) left one stale line behind: `_build_pipeline()` still called `self._webrtcbin.sync_state_with_parent()` at its very end - correct when `webrtcbin` was built there too (pre-fix), meaningless now that it isn't built until an offer arrives. Every verification pass in this doc used the synthetic test-pattern camera (`CAMERA_TEST_PATTERN_FALLBACK=true`) and never noticed: the `AttributeError` fires once, on the very first camera frame, gets caught and logged by `agent.py`'s own `_on_camera_frame` handler, and - because `self._pipeline` is already assigned before the crash - every frame after the first skips `_build_pipeline()` entirely and pushes straight into an already-`PLAYING` pipeline. Signalling and video still worked end to end despite it, which is exactly how it went unnoticed for a full milestone: a real ERROR-level stack trace in the logs on every single startup, with zero observable effect on anything this doc's own verification checked. Testing against a real, physically-attached webcam (not the synthetic pattern) surfaced it immediately, purely because it happened to be the next real hardware path exercised. Fixed by deleting the line - `_build_pipeline()` no longer references `webrtcbin` at all, matching its own docstring's claim ("no webrtcbin exists yet at all") exactly instead of almost.

### Fourth pass (post-Milestone-11): the reconnect race was narrowed, not closed

Building the LiDAR feature meant rapid iteration on the Robot page - reload, navigate away and back, over and over, dozens of times per hour rather than the few times per test run this doc's own verification exercised. That volume surfaced what the second pass's fix actually was: a *narrowing* of the reconnect race, not a full close. Roughly 1 in 10-15 rapid reconnects still hit `GStreamer pipeline error: ... streaming stopped, reason not-linked` - and, worse, once it happened the encode chain stayed broken for every *subsequent* offer too (only the downstream `webrtcbin` gets rebuilt per offer; the upstream `appsrc → videoconvert → x264enc → rtph264pay → capsfilter` chain is a singleton for the process's whole lifetime, and a GStreamer element that's posted a bus `ERROR` doesn't just resume streaming because something downstream got relinked).

Root cause: `appsrc` streams on its **own** internal GStreamer thread, entirely independent of the GLib main loop `_prepare_fresh_webrtcbin()` runs on (see `_run_on_main_loop`). A camera frame already queued inside `appsrc` can be mid-flight through the encode chain at the exact moment `_prepare_fresh_webrtcbin()` unlinks `rtpcaps`'s src pad to swap in a fresh `webrtcbin` - two independent threads touching the same pad with no coordination between them. `self._negotiating` (an `Event` checked before *scheduling* a push) narrows this window but can't close it: the check and the actual `push-buffer` call are separated by a `GLib.idle_add()` hop, so a frame can pass the check and only actually run after negotiation has since started.

**The fix:** the standard GStreamer pattern for reconfiguring a pipeline while it's `PLAYING` (see GStreamer's own "Dynamic Pipelines" tutorial) - install a `Gst.PadProbeType.BLOCK_DOWNSTREAM` probe on `rtpcaps`'s src pad *before* unlinking it, wait (bounded, like every other wait in this file) for the probe to confirm the pad is actually quiesced, do the entire unlink/teardown/relink while it's blocked, then remove the probe. This guarantees no buffer is in flight through that pad for the whole operation, closing the race at its actual source instead of narrowing the window a second time. Verified with a dedicated stress test - not just "run the suite again": 16 reconnects in rapid succession (reload, ~0.3s pause, repeat), asserting real decoding video after every single one. Before the fix: 2 succeeded, then 14 consecutive failures once the encode chain wedged. After: 16/16, `rtp_packets_sent` climbing continuously, zero `webrtc_offers_failed`.

### Full end-to-end pass (original Milestone 9 verification, before the fourth pass above)
