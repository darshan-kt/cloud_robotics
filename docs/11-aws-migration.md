...
|---|---|---|
| `mosquitto` (Eclipse Mosquitto) | **AWS IoT Core** | Broker enforcement moves from `password_file`/`acl_file` to IoT policies expressing the same per-topic rules (see above). Robot connects over MQTT/TLS on 8883 instead of plaintext 1883 — `MQTT_HOST`/`MQTT_PORT` env vars, already how this is configured, are the only code-adjacent change. Every topic added since this guide was first written - `lidar` included - rides the exact same `robots/{id}/...` pattern already accounted for here; a new topic is a new IoT policy line, never a new AWS resource. |
